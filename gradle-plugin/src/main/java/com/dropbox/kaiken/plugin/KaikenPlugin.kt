...
open class KaikenPlugin : Plugin<Project> {
  override fun apply(project: Project) {
    project.extensions.create("Kaiken", KaikenExtension::class.java)
    ...
    // Apply the Kaiken plugin after the Kotlin plugin was applied. There could be a timing
    ...
    project.afterEvaluate {
      if (!once.get()) {
        throw GradleException(
            "No supported plugins for Kaiken found on project " +
        ...
        .configureEach { compileTask ->
          ...
          compileTask.doFirst {
            ...
            compileTask.logger.info(
                "Kaiken: Use precise java tracking: ${compileTask.usePreciseJavaTracking}"
            )
          }
        }
  }
        ...
        .configureEach { stubsTask ->
          ...
          stubsTask.doFirst {
            ...
            stubsTask.logger.info(
                "Kaiken: Incremental compilation enabled: ${stubsTask.incremental} (stub)"
            )
          }
        }
  ...
  private fun disableIncrementalCompilationAction(
    ...
    // This will trigger a full compilation of a module using Kaiken even though its
    ...
    val disableIncrementalCompilationTaskProvider = project.tasks.register(
        compileTaskName + "CheckIncrementalCompilationKaiken",
    ...
    project.tasks.named(compileTaskName, KotlinCompile::class.java) { compileTask ->
      compileTask.doFirst {
        ...
        compileTask.logger.info(
            "Kaiken: Incremental compilation enabled: ${compileTask.incremental} (compile)"
        )
      }
    }
  }
