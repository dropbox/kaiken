...
fun generateAnvilParts(annotatedActivityTypeElement: TypeElement, elements: Elements, annotatedActivityType: TypeMirror, pack: String, filer: Filer): Pair<com.squareup.kotlinpoet.ClassName?, com.squareup.kotlinpoet.ClassName?> {
    ...
    if (injectableAnnotationComponent != null && anvilOnPath()) {
        ...
        anvilScope = scope?.asTypeName() as com.squareup.kotlinpoet.ClassName
    }
...
private fun dependenciesValue(annotatedActivityTypeElement: TypeElement): com.squareup.kotlinpoet.ClassName? {
    return try {
        annotatedActivityTypeElement
