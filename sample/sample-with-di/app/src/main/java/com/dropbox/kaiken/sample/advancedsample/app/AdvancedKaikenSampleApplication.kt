...
import com.dropbox.kaiken.skeleton.usermanagement.AllUsersFlow
...
class BindingModule {
    ...
    @Provides
    @SingleIn(AppScope::class)
    fun provideSkeletonUserFlow(accountStore: AccountStore, mapper: SkeletonMapper<DiSampleUser>): AllUsersFlow {
        return accountStore.getAllUsers().map { it.map { mapper.toSkeletonUser(it) }.toSet() } as AllUsersFlow
    }
}
